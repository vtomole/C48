...
char *construct_ifelse(stack *op_stack, stack *num_stack){
  ...
  char *num1 = (char*)pop(num_stack);
  char *expr = malloc(sizeof(char)*100);
  sprintf(expr, "(if %s %s %s)", num1, num2, num3);
  // sprintf(expr, "%s %s %s", op, num1, num2); 
  free(num1);
  free(num2);
  free(num3);
...
int precedence(char *op) {
  if(isequal(op, "(") || isequal(op, ")") || isequal(op, "{") || isequal(op, "}")){
    return 1;
  }
...
int is_keyword(char *op){
  ...
  keyword |= is_ifelse(op) || isequal(op, "set") || isequal(op, "fun");
  return keyword;
}
